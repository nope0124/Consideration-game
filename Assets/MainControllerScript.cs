...
public class MainControllerScript : MonoBehaviour
{
    ...
    static int[] saveCount = {7, 6, 2};
    static int[] blueVec;
    static int[] yellowVec;
    static int[] redVec;
    GameObject[] bluePawn;
    GameObject[] yellowPawn;
    GameObject[] redPawn;
    static string[] colorName = {"青", "黄", "赤"};
    int runCount = 1;
    int curCount = 0;
    ...
    public GameObject selectCenter;
    ...
    void Start()
    {
        saveCount[0] = NumberScript.getBlueCount();
        saveCount[1] = NumberScript.getYellowCount();
        saveCount[2] = NumberScript.getRedCount();
    ...
    void Update()
    {
        ...
        //     if (405 <= mousePosition.y && mousePosition.y <= 700) {
        //         if (200 <= mousePosition.x && mousePosition.x < 600) curCount = 0;
        //         if (600 <= mousePosition.x && mousePosition.x < 1000) curCount = 1;
        //         if (1000 <= mousePosition.x && mousePosition.x < 1400) curCount = 2;
        //         fucMaterial();
        //     }
        // }
        ...
        if (!isOver) {
            ...
            if (curCount == 0) {
                ...
                selectCenter.SetActive(false);
                selectRight.SetActive(false);
            } else if (curCount == 1) {
                selectLeft.SetActive(false);
                selectCenter.SetActive(true);
                selectRight.SetActive(false);
            } else if (curCount == 2) {
                selectLeft.SetActive(false);
                selectCenter.SetActive(false);
    ...
    public void run() {
        if (!isTurn || isOver) return; 
        isTurn = false;
        countDown = 60 + Random.Range(0, 60);
